...
-- Sector is part of the context a candidate supplies for their search. Keep it
-- fixed across every comparison tier; only experience, region, and seniority
-- may relax when a five-contributor cohort cannot be formed.
...
begin
  ...
  select episode.id, episode.role_id, episode.sector_id, episode.role_level,
  ...
  with candidates as (
    select episode.id, submission.data_subject_id, episode.role_id, episode.sector_id,
