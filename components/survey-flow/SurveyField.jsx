...
export function SurveyField({ children, className = '', error, hideError = false, hint, id, label, labelAccessory, required = false }) {
  const hintId = hint ? `${id}-hint` : undefined
  const errorId = error && !hideError ? `${id}-error` : undefined
  const describedBy = [hintId, errorId].filter(Boolean).join(' ') || undefined

  return (
    <div className={className}>
      <div className="flex items-baseline justify-between gap-3">
        <label htmlFor={id} className={`text-sm font-semibold ${error ? 'text-danger' : 'text-ink'}`}>
