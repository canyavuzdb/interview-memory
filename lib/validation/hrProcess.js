...
export function validateHRProcessStep(step, state, messages) {
  ...
  if (step === 1) {
    if (!['catalog', 'not_listed'].includes(state.companySelection)) errors.companyName = messages.required
    if (state.companySelection === 'catalog' && state.companyName.trim().length < 2) errors.companyName = messages.required
    if (state.appliedRole.trim().length < 2) errors.appliedRole = messages.required
    if (!required(state.seniority)) errors.seniority = messages.required
